...
from pylock.validator import resolve_installed_package_info, check_package_availability, validate_environment
...
def test_validate_passes(monkeypatch):
    monkeypatch.setattr("pylock.validator.check_package_availability", lambda dep, ver=None: {
        'available': True,
    ...
    validate_environment(lockfile)  # should not raise

def test_validate_missing_package(monkeypatch):
    monkeypatch.setattr("pylock.validator.check_package_availability", lambda dep, ver=None: {
        'available': False,
    ...
    lockfile = {'deps': {'flask': {'version': '2.0.0'}}}
    with pytest.raises(RuntimeError, match="Missing required package: flask"):
        validate_environment(lockfile)

def test_validate_version_mismatch_user_accepts(monkeypatch):
    monkeypatch.setattr("pylock.validator.check_package_availability", lambda dep, ver=None: {
        'available': True,
        'version_matches': False,
        'version': '1.0.0',
...
def test_validate_version_mismatch_user_declines(monkeypatch):
    monkeypatch.setattr("pylock.validator.check_package_availability", lambda dep, ver=None: {
        'available': True,
        'version_matches': False,
        'version': '1.0.0',
